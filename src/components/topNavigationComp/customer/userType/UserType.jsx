...
import { deleteUserType, listUserType, paginationUserTypeMaster } from '../../../../api/login/Login';
...
const UserType = () => {
   ...
   const [count, setCount] = useState(10)
    ...
    const getPaginationApi = async (page) => {
        setLoading(true)
        try {
            const res = await paginationUserTypeMaster(page, count)
            // console.log(res?.data?.area);
            setTotalCount(res?.data?.count)
            setData(res?.data?.usertype)
        } catch (error) {
